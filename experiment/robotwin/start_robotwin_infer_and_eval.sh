...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --recap_adapter_registry task-to-adapter JSON; missing tasks use null/base"
...
if [ -n "$recap_adapter_path" ] && [ -n "$recap_adapter_registry" ]; then
    echo -e "\033[31mError: use only one of --recap_adapter_path and --recap_adapter_registry\033[0m"
    exit 1
fi
...
if [ -n "$recap_adapter_registry" ] && [ ! -f "$recap_adapter_registry" ]; then
    echo -e "\033[31mError: --recap_adapter_registry is not a file: ${recap_adapter_registry}\033[0m"
    exit 1
fi
...
if [ -n "$counterfactual_policy_decision_map" ] && [ ! -f "$counterfactual_policy_decision_map" ]; then
    echo -e "\033[31mError: --counterfactual_policy_decision_map is not a file: ${counterfactual_policy_decision_map}\033[0m"
    exit 1
fi
...
for slot in $(seq 0 $((num_slots-1))); do
    ...
    if [ -n "$counterfactual_policy_decision_map" ]; then
        counterfactual_map_arg="--counterfactual_policy_decision_map '${counterfactual_policy_decision_map}'"
    fi
    recap_adapter_arg=""
    if [ -n "$recap_adapter_path" ]; then
        recap_adapter_arg="--recap_adapter_path '${recap_adapter_path}'"
    elif [ -n "$recap_adapter_registry" ]; then
        recap_adapter_arg="--recap_adapter_registry '${recap_adapter_registry}'"
    fi
