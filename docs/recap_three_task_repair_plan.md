...
**2026-09-10 更新：** 600-input GPU gate 和 36-cell P0 控制重训均已完成并通过
完整性/数值审计，见 [P0 实施和完整结果](recap_training_ablation_20260910.md)。
旧 holdout surrogate 没有跨任务/seed 一致改善，三任务闭环收益仍未证实。
100-pair 全决策头部图的粗粒度视觉标注已完成并封存，见
[阶段审阅及限制](recap_failure_stage_audit_20260910.md)。这是单 AI、显式条件映射盲审；
不是全腕相机/数值动作/动力学审计，未知阶段仍保留。随后按收缩路线完成了
[现有 P0 正式模型的一次性小规模闭环检查](recap_repaired_policy_check_result_20260911.md)：
120 个新配对状态中，三任务等权 delta −1.67 pp，95% CI [−10.0,+6.67] pp，未通过
预定的 +5 pp/无任务净退化优先级旗标。该小样本不证明无效，但不支持直接扩大验证。
P1/P2 尚未启动或排队。
原最终实验已经完整结束并通过独立统计审计，见 [最终结果](recap_final_result_20260910.md)；
以下研究边界和门槛仍然有效，历史“正在运行”描述不代表当前状态。
...
1. **已完成**：原始证据审计、P0 代码/CPU 测试及独立 probe 工具；没有新训练/模拟器调用。
